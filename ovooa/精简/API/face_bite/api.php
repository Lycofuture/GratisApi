...
class face_bite
{
	...
	private function parameterException()
	{
		...
		if(file_exists(__DIR__.'/cache/'.$info['QQ'].'.GIF') && $this->is_image(__DIR__.'/cache/'.$info['QQ'].'.GIF'))
		{
			...
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_bite/cache/'.$this->info['QQ'].'.GIF'));
			return;
		}
		$this->file = $info['QQ'].'.GIF';
		$this->filecache = __DIR__.'/cache/'.$info['QQ'].'.GIF';
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$image->Newimage(362, 364, 'white');//设置图片大小颜色
				...
				$imagick = New imagick('./bite/'.$k);//创建首层图
