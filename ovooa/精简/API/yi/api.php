...
New yi(array('QQ'=>$QQ, 'type'=>$type, 'url'=>$url));
...
class yi
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/yi/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/yi/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
		...
		if(!is_array($file) || !$file)
		{
			...
			foreach($file as $key=>$file)
			{
				if(file_exists($file))
				{
					...
					$Image->setformat('gif');
					$Image->modulateImage(100, 0, 100);//设置为黑白色
					$GIF->addImage($Image);
					if($this->Delay)
					{
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/yi/cache/'.$this->file));
			return;
		}
	}
	...
	public function __destruct()
	{
		...
		$this->delfile(__DIR__.'/gif/', $this->fileTime);
	}
}