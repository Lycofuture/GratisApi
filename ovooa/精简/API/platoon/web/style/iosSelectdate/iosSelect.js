	...
	IosSelect.prototype = {
		...
		setScorllEvent: function (scrollInstance, index) {
			...
			scrollInstance.on('scrollEnd', function () {
				...
				numFlag = $(".at").eq(1).html().substr(-2,2);
				if(index == 1){
					showDateDom.attr('data-year', obj.id);
				}else if(index == 2){
					showDateDom.attr('data-month', obj.id);
				}else if(index == 3){
					showDateDom.attr('data-date', obj.id);
				}else{
					showDateDom.attr('data-time', obj.id);
				}
