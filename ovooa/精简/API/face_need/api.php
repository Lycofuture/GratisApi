...
New face_need(array('QQ'=>$QQ, 'type'=>$type, 'url'=>$url));
...
class face_need
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_need/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_need/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$image->Newimage(500, 440, 'white');//设置图片大小颜色
				...
				$imagick = New imagick('./need/'.$k);//创建首层图
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_need/cache/'.$this->file));
			return;
		}
	}
