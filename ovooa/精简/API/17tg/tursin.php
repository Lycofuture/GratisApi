...
$url="https://web.qun.qq.com/cgi-bin/media/set_media_state?t=0.85959781718529&g_tk=".$gtk."&state=1&gcode=".$group."&qua=V1_AND_SQ_8.4.1_1442_YYB_D&uin=".$qq."&format=json&inCharset=utf-8&outCharset=utf-8";
$post="0";
...
if($retcode=="0"){
    Switch($type){
        case 'text':
        need::send('已开启一起听歌！','text');
        break;
        default:
        need::send(array("code"=>"1","text"=>"已开启一起听歌!"));
        break;
    }
}else
if($retcode=="100061"){
    Switch($type){
        case 'text':
        need::send('已开启，请勿重试！','text');
        break;
        default:
        need::send(array("code"=>"-4","text"=>"已开启，请勿重试！"));
        break;
    }
}else
if($retcode=="100051"){
    Switch($type){
        ...
        break;
        defaul:
        need::send(array("code"=>"-5","text"=>"开启失败，权限不足！"));
        break;
    }
}else
if($retcode=="100000"){
    Switch($type){
        ...
        break;
        default:
        need::send(array("code"=>"-6","text"=>"开启失败，SKEY已失效!"));
        break;
    }
}else{
    Switch($type){
        ...
        break;
        default:
        need::send(array("code"=>"-7","text"=>"开启失败，未知错误!"));
        break;
    }
}
