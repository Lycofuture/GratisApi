...
New face_play(array('QQ'=>$QQ, 'url'=>$url, 'type'=>$type));
...
class face_play
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_play/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_play/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$image->Newimage(480, 400, 'white');//设置图片大小颜色
				...
				$imagick = New imagick('./play/'.$k);//创建首层图
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_play/cache/'.$this->file));
			return;
		}
	}
