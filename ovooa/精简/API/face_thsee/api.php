...
New face_thsee(array('QQ'=>$QQ, 'type'=>$type, 'url'=>$url));
...
class face_thsee
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_thsee/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_thsee/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$image->Newimage(707, 1000, 'white');//设置图片大小颜色
				...
				$imagick = New imagick('./thsee/'.$k);//创建首层图
				$imagick->setimageformat('png');//设置png
				$imagick->setimagematte(true);//激活遮罩通道
				// $this->修改($imagick, 707, 1000);
				// $this->put('./0.png', $imagick->getImageBlob());
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_thsee/cache/'.$this->info['QQ'].'.GIF'));
			return;
		}
	}
