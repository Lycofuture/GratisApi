document.onreadystatechange = function () {
    if (document.readyState === "complete") {
        const loading = document.querySelector('.fan-loding');
...
fetch('./API/rl/api.php')
.then(res=>res.json())
    .then(json=>{
        if(json.code != 1){
            return;
        }else{
            if(json.data.knob == '清明' || json.data.AD.indexOf('11月30日') != -1 || json.data.lunar.indexOf('冬月初七') != -1){
...
function music(){
    $("#musicplayer").append("<div id=\"music\" key=\"60f95b4baa88e\" api=\"http://music.ovooa.com\"></div><script id=\"xplayer\" src=\"http://music.ovooa.com/api/PlayerJs/id/60f95b4baa88e\" ></script>");
}
