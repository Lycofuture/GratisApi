...
New face_yu(array('QQ'=>$QQ, 'type'=>$type, 'url'=>$url));
...
class face_yu
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_yu/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_yu/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$image->Newimage(600, 600, 'white');//设置图片大小颜色
				$image->setimageformat('png');//设置图片格式为png
				// $image->setimagematte(true);//激活遮罩通道
				if(isset($v['位置'][0][0]) && isset($v['大小'][0][0]))
				{
					...
					$image->compositeimage($imagick, Imagick::COMPOSITE_ATOP, $v['位置'][1][0], $v['位置'][1][1]);//合并头像以及背景
					$imagick->destroy();//关闭头像
					unset($imagick);
				}
				$imagick = New imagick('./yu/'.$k);//创建首层图
				$imagick->setimageformat('png');//设置png
				$imagick->setimagematte(true);//激活遮罩通道
				$image->compositeimage($imagick, Imagick::COMPOSITE_ATOP, 0, 0);//合并图层
				// $this->修改($imagick, 600, 600);
				// $this->put('./1.png', $imagick);
				$imagick->destroy();//关闭图片
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_yu/cache/'.$this->file));
			return;
		}
	}
