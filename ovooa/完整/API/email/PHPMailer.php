...
class PHPMailer
{
     ...
     * It is called out by need::send() for each email sent.
     *
     ...
     * In need::send(), valid and non duplicate entries are moved to $all_recipients
     ...
     * In need::send(), valid and non duplicate entries are moved to $ReplyTo.
     * This array is used only for addresses with IDN.
     *
     ...
     * be modified after calling this function), addition of such addresses is delayed until need::send().
    ...
    public function setFrom($address, $name = '', $auto = true)
    {
        $address = trim($address);
        $name = trim(preg_replace('/[\r\n]+/', '', $name)); //Strip breaks and trim
        // Don't validate now addresses with IDN. Will be done in need::send().
        $pos = strrpos($address, '@');
        if (false === $pos or
     ...
     * Before the need::send() call, queued addresses (i.e. with IDN) are not yet included.
     *
     * @return array
     */
