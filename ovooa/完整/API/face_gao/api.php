...
New face_gao(array('QQ'=>$QQ, 'type'=>$type, 'url'=>$url));
...
class face_gao
{
	...
	private function parameterException()
	{
		...
		if(isset($info['QQ']) && $info['QQ'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_gao/cache/'.$info['QQ'].'.GIF'));
				return;
			}
		}else
		if(isset($info['url']) && $info['url'])
		{
			...
			{
				$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_gao/cache/'.Md5($info['url']).'.GIF'));
				return;
			}
		}else{
	...
	private function 开始合成($file)
	{
		...
		if(!file_exists($file))
		{
			...
			foreach($bg as $k=>$v)
			{
				...
				$imagick = New imagick('./gao/'.$k);//创建首层图
			...
			$GIF->destroy();//关闭GIF
			// exit;
			$this->evaluation(array('code'=>1, 'text'=>'http://ovooa.com/API/face_gao/cache/'.$this->file));
			return;
		}
	}
